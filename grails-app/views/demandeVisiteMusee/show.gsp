	...
	<body>
		...
		<div id="show-demandeVisiteMusee" class="content scaffold-show" role="main">
			...
			<ol class="property-list demandeVisiteMusee">
				...
				<g:if test="${demandeVisiteMuseeInstance?.demande}">
				<li class="fieldcontain">
					<span id="demande-label" class="property-label"><g:message code="demandeVisiteMusee.demande.label" default="Demande" /></span>
					
						<span class="property-value" aria-labelledby="demande-label"><g:link controller="demandeVisite" action="show" id="${demandeVisiteMuseeInstance?.demande?.id}">${demandeVisiteMuseeInstance?.demande?.encodeAsHTML()}</g:link></span>
